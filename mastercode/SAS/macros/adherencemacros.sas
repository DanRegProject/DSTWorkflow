...
$Revision: 333 $
$Author: wnm6683 $
$Id: trtPeriods.sas 333 2022-05-12 07:38:22Z wnm6683 $
*/
/*
  #+NAME
    %trtPeriods
...
*/
%macro trtPeriods(indata, outdata, drug, type, IndexDate=, ajour=today(), slipscale=1.5, slipdays=1, tabsprday=, stddosage=, maxdosage=, mindosage=0.1, InclusionDays=,subset=,bydrug=FALSE,dosedata=);
  %if &type=1 and &tabsprday= %then %do;
      %put ERROR trtPeriods: tabsprday must be specified if type=1;
      %abort cancel;
      %end;
  %if &type=2 and &stddosage= %then %do;
      %put ERROR trtPeriods: stddosage must be specified if type=2;
      %abort cancel;
      %end;
  %if &type=2 and &dosedata ne %then %do;
      %if %varexist(&dosedata,&stddosage) = 0 %then %do;
		%put ERROR trtPeriods: stddosage must be included in &dosedata if type=2;
      	%abort cancel;
		proc sql;
...
%macro jointrtPeriods(outdata,name,list,postfix,inlib=work,indatalist=);
...
%macro mergetrtPeriods(basedata,perdata,indexdate,pervar,postfix=);
